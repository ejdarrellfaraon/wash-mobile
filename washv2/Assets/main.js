...
var url="http://localhost/washv2/Assets/api";
...
function login(uname,pword){
	...
	fetch(url+'/tbl_userreg'+uname).then(res=>res.json()).then(function(data){
		var ls = "";
		if(data.length <= 0){
			...
			for (var i = 0; i < data.length; i++) {
				if(data[i].fld_status == 'Active'){
					if (data[i].fld_email == uname && data[i].fld_password == pword) {
						...
						localStorage.setItem('usercnumber', data[i].fld_contact);
						localStorage.setItem('userpassword', data[i].fld_password);
					// localStorage.setItem('profilepic', data[i].fld_contact);
					// localStorage.setItems('', data[i].password);
					window.location = "../mainpage.html";
				} 
				// else if(data[i].fld_email == uname && data[i].fld_password == pwd && data[i].fld_accountstatus == "Deactivated"){
				// 	window.alert("Your account has been deactivated as a result of violating our application's policies. Please contact customer support for more information.\n\n09285838025");
				// }
				else {
					alert("Invalid username or password. Please try again.");
					
				}
			}else if(data[i].fld_status == 'Deactivate'){
